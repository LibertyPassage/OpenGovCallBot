...
def save_event_view():
    ...
    try:
        ...
        if len(event_summary) > 600:
            return jsonify(status='error', message='Event Summary must not exceed 600 words.')
        ...
        clean_text = lambda text: text.replace(',', '.').replace('\n', ' ').replace("'", '').replace('’', '').replace('"', '').replace(':', '').replace('”', '')

        if event_summary:
            event_summary = clean_text(event_summary)

        if event_location:
            event_location = clean_text(event_location)

        if event_name:
            event_name = clean_text(event_name)

        if event_industry:
            event_industry = clean_text(event_industry)
        ...
        cursor.execute(
            f"INSERT INTO {registration_table} (eventName, eventLocation, eventSummary, eventDate, eventTime, eventIndustry, attendees) VALUES (%s, %s, %s, %s, %s, %s, %s)",
